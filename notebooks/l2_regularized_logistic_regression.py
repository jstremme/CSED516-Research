...
def accuracies(pred_vals, y):
    '''
    Calculate accuracies for many predictions.
    Input:
        - pred_vals: Array of array of predictions
        - y: Target labels
    Output:
        - Array of accuracies for each prediction
    '''

    return np.array([accuracy(pred, y) for pred in pred_vals])

def plot_accuracies(pred_vals, y):
    '''
    Plot the accuracy at each iteration of gradient descent.
    Input:
      - pred_vals: Values the gradient descent algorithm stepped to
      - y: Target labels
      - labels: Labels for each line plotted
    '''

    a = accuracies(pred_vals, y)
    print('Accuracy at last iteration: {}'.format(a[-1]))
    ...
    plt.title('Accuracy at Each Iteration of Gradient Descent', fontsize=16)
    plt.show()
